...
NAN_METHOD(Kerberos::AuthGSSClientInit) {
    const char* usage =
        "Requires a service string uri, integer flags, string credentialsCache and a callback "
        "function";

    // Ensure valid call
    if (info.Length() != 4)
        return Nan::ThrowError(usage);
    if (!info[0]->IsString() || !info[1]->IsInt32() || !info[2]->IsString() ||
        !info[3]->IsFunction())
        return Nan::ThrowError(usage);
    ...
    uv_queue_work(
        uv_default_loop(), &worker->request, Kerberos::Process, (uv_after_work_cb)Kerberos::After);
...
NAN_METHOD(Kerberos::AuthGSSClientStep) {
    ...
    if (info.Length() != 2 && info.Length() != 3)
        return Nan::ThrowError(
            "Requires a GSS context, optional challenge string and callback function");
    if (info.Length() == 2 && (!KerberosContext::HasInstance(info[0]) || !info[1]->IsFunction()))
        return Nan::ThrowError(
            "Requires a GSS context, optional challenge string and callback function");
    if (info.Length() == 3 &&
        (!KerberosContext::HasInstance(info[0]) || !info[1]->IsString() || !info[2]->IsFunction()))
        return Nan::ThrowError(
            "Requires a GSS context, optional challenge string and callback function");
...
NAN_METHOD(Kerberos::AuthGSSClientUnwrap) {
    ...
    // Allocate a structure
    AuthGSSClientUnwrapCall* call =
        (AuthGSSClientUnwrapCall*)calloc(1, sizeof(AuthGSSClientUnwrapCall));
    if (call == NULL)
        die("Memory allocation failed");
    call->context = kerberos_context;
    call->challenge = challenge_str;
    ...
    Local<Function> callbackHandle =
        info.Length() == 3 ? Local<Function>::Cast(info[2]) : Local<Function>::Cast(info[1]);
    Nan::Callback* callback = new Nan::Callback(callbackHandle);
...
NAN_METHOD(Kerberos::AuthGSSClientWrap) {
    ...
    if (info.Length() != 3 && info.Length() != 4)
        return Nan::ThrowError(
            "Requires a GSS context, the result from the authGSSClientResponse after "
            "authGSSClientUnwrap, optional user name and callback function");
    if (info.Length() == 3 &&
        (!KerberosContext::HasInstance(info[0]) || !info[1]->IsString() || !info[2]->IsFunction()))
        return Nan::ThrowError(
            "Requires a GSS context, the result from the authGSSClientResponse after "
            "authGSSClientUnwrap, optional user name and callback function");
    if (info.Length() == 4 && (!KerberosContext::HasInstance(info[0]) || !info[1]->IsString() ||
    ...
    // Unpack the callback
    Local<Function> callbackHandle =
        info.Length() == 4 ? Local<Function>::Cast(info[3]) : Local<Function>::Cast(info[2]);
    Nan::Callback* callback = new Nan::Callback(callbackHandle);
...
NAN_METHOD(Kerberos::AuthGSSClientClean) {
    // Ensure valid call
    if (info.Length() != 2)
        return Nan::ThrowError("Requires a GSS context and callback function");
    if (!KerberosContext::HasInstance(info[0]) || !info[1]->IsFunction())
        return Nan::ThrowError("Requires a GSS context and callback function");
    ...
    // Allocate a structure
    AuthGSSClientCleanCall* call =
        (AuthGSSClientCleanCall*)calloc(1, sizeof(AuthGSSClientCleanCall));
    if (call == NULL)
        die("Memory allocation failed");
    call->context = kerberos_context;
...
static void _authGSSServerInit(Worker* worker) {
    ...
    response = authenticate_gss_server_init(
        call->service, call->constrained_delegation, call->username, state);
...
NAN_METHOD(Kerberos::AuthGSSServerInit) {
    ...
    if (info.Length() != 4)
        return Nan::ThrowError(
            "Requires a service string, constrained delegation boolean, a username string (or "
            "NULL) for S4U2Self protocol transition and a callback function");

    if (!info[0]->IsString() || !info[1]->IsBoolean() ||
        !(info[2]->IsString() || info[2]->IsNull()) || !info[3]->IsFunction())
        return Nan::ThrowError(
            "Requires a service string, constrained delegation boolean, a username string (or "
            "NULL) for S4U2Self protocol transition and  a callback function");

    if (!info[1]->BooleanValue() && !info[2]->IsNull())
        return Nan::ThrowError("S4U2Self only possible when constrained delegation is enabled");
...
NAN_METHOD(Kerberos::AuthGSSServerClean) {
    ...
    // Allocate a structure
    AuthGSSServerCleanCall* call =
        (AuthGSSServerCleanCall*)calloc(1, sizeof(AuthGSSServerCleanCall));
    if (call == NULL)
        die("Memory allocation failed");
    call->context = kerberos_context;
...
NAN_METHOD(Kerberos::AuthGSSServerStep) {
    // Ensure valid call
    if (info.Length() != 3)
        return Nan::ThrowError("Requires a GSS context, auth-data string and callback function");
    if (!KerberosContext::HasInstance(info[0]))
        return Nan::ThrowError("1st arg must be a GSS context");
    if (!info[1]->IsString())
        return Nan::ThrowError("2nd arg must be auth-data string");
    if (!info[2]->IsFunction())
        return Nan::ThrowError("3rd arg must be a callback function");
...
static void _authUserKrb5Password(Worker* worker) {
    ...
    gss_client_response* response =
        authenticate_user_krb5_password(call->username, call->password, call->service);
...
NAN_METHOD(Kerberos::AuthUserKrb5Password) {
    const char* usage =
        "Requires a username (string), password (string), service (string) and callback "
    ...
    AuthUserKrb5PasswordCall* call =
        (AuthUserKrb5PasswordCall*)calloc(1, sizeof(AuthUserKrb5PasswordCall));
    if (call == NULL)
        die("Memory allocation failed");
...
void Kerberos::After(uv_work_t* work_req) {
    ...
    if (worker->error) {
        Local<Value> err =
            v8::Exception::Error(Nan::New<String>(worker->error_message).ToLocalChecked());
