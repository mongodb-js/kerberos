...
const MongoAuthProcess = function(host, port, service_name, options) {
 ...
 *
 *******************************************************************/
const Win32MongoProcessor = function(host, port, service_name, options) {
  options = options || {};
  this.host = host;
  this.port = port;
...
Win32MongoProcessor.prototype.init = function(username, password, callback) {
  ...
  performGssapiCanonicalizeHostName(this.gssapiCanonicalizeHostName, this.host, function(err) {
    if (err) return callback(err);
    // Acquire credentials
    self.ssip.SecurityCredentials.aquire_kerberos(username, password, function(
      err,
      ...
      // Set up target
      self.target = `${self.gssapiServiceName}/${self.host}`;

      // Do we have a service realm
      if (self.gssapiServiceRealm) {
        self.target = `${self.target}@${self.gssapiServiceRealm}`;
      }
...
Win32MongoProcessor.first_transition = function(self) {
  return function(payload, callback) {
    self.ssip.SecurityContext.initialize(self.security_credentials, self.target, payload, function(
      err,
      security_context
    ) {
      if (err) return callback(err);
...
Win32MongoProcessor.third_transition = function(self) {
  return function(payload, callback) {
    ...
    // Get the raw bytes
    const encryptedBytes = new Buffer(payload, 'base64');
    const encryptedMessage = new Buffer(messageLength);

    // Copy first byte
    encryptedBytes.copy(encryptedMessage, 0, 0, messageLength);

    // Set up trailer
    const securityTrailerLength = encryptedBytes.length - messageLength;
    const securityTrailer = new Buffer(securityTrailerLength);

    // Copy the bytes
    encryptedBytes.copy(securityTrailer, 0, messageLength, securityTrailerLength);

    // Types used
    const SecurityBuffer = self.ssip.SecurityBuffer;
    const SecurityBufferDescriptor = self.ssip.SecurityBufferDescriptor;
    ...
    const descriptor = new SecurityBufferDescriptor(buffers);
    ...
    self.security_context.decryptMessage(descriptor, function(err /* , security_context */) {
      ...
      const bytesReceivedFromServer = new Buffer(length);
      ...
      if (self.username != null) {
        const authorization_id_bytes = new Buffer(self.username, 'utf8');
      ...
      self.security_context.queryContextAttributes(0x00, function(err, sizes) {
        ...
        self.security_context.encryptMessage(descriptor, 0x80000001, function(
          err,
          security_context
        ) {
          if (err) return callback(err);
 ...
 *
 *******************************************************************/
const UnixMongoProcessor = function(host, port, service_name, options) {
  options = options || {};
  this.host = host;
  this.port = port;
...
UnixMongoProcessor.prototype.init = function(username, password, callback) {
  ...
  performGssapiCanonicalizeHostName(this.gssapiCanonicalizeHostName, this.host, function(err) {
    ...
    self.kerberos.authGSSClientInit(self.target, self.Kerberos.GSS_C_MUTUAL_FLAG, function(
      err,
      context
    ) {
...
UnixMongoProcessor.third_transition = function(self) {
  return function(payload, callback) {
    ...
    self.kerberos.authGSSClientUnwrap(self.context, payload, function(err /* , result */) {
      ...
      // Wrap the response
      self.kerberos.authGSSClientWrap(self.context, self.context.response, self.username, function(
        err
        /* , result */
      ) {
        if (err) return callback(err, false);
