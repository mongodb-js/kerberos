 ...
 * @param {function} callback
 */
function authGSSClientResponse(context, callback) {
  validateParameter(context, { name: 'context', type: 'object' });
  validateParameter(callback, { name: 'callback', type: 'function' });

  kerberos.authGSSClientResponse(context, callback);
}

/**
 * Returns true if confidentiality was enabled in the previously unwrapped
 * buffer, and false if not.
 *
 * @param {KerberosContext} context The context object returned from `authGSSClientInit`
 * @param {function} callback
 */
function authGSSClientResponseConf(context, callback) {
  validateParameter(context, { name: 'context', type: 'object' });
  validateParameter(callback, { name: 'callback', type: 'function' });

  kerberos.authGSSClientResponseConf(context, callback);
}

/**
 * Get the user name of the principal authenticated via the now complete GSSAPI
 * client-side operations. This method must only be called after authGSSClientStep
 * returns a complete response code.
 *
 * @param {KerberosContext} context The context object returned from `authGSSClientInit`
 * @param {function} callback
 */
function authGSSClientUserName(context, callback) {
  validateParameter(context, { name: 'context', type: 'object' });
  validateParameter(callback, { name: 'callback', type: 'function' });

  kerberos.authGSSClientUserName(context, callback);
}
 ...
 * @param {function} callback
 */
function authGSSServerResponse(context, callback) {
  validateParameter(context, { name: 'context', type: 'object' });
  validateParameter(callback, { name: 'callback', type: 'function' });

  kerberos.authGSSServerResponse(context, callback);
}
 ...
 * @param {function} callback
 */
function authGSSServerUserName(context, callback) {
  validateParameter(context, { name: 'context', type: 'object' });
  validateParameter(callback, { name: 'callback', type: 'function' });

  kerberos.authGSSServerUserName(context, callback);
}
 ...
 * @param {function} callback
 */
function authGSSServerTargetName(context, callback) {
  validateParameter(context, { name: 'context', type: 'object' });
  validateParameter(callback, { name: 'callback', type: 'function' });

  kerberos.authGSSServerTargetName(context, callback);
}
