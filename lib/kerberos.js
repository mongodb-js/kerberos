...
const kerberos = require('../build/Release/kerberos');
const KerberosNative = kerberos.Kerberos;

const Kerberos = function() {
...
// callback takes two arguments, an error string if defined and a new context
//
Kerberos.prototype.authGSSServerInit = function(
  service,
  ...
  if (typeof constrained_delegation === 'string') {
    throw new Error(
      'S4U2Self protocol transation is not possible without enabling constrained delegation'
    );
  }
  ...
  return this._native_kerberos.authGSSServerInit(
    service,
    constrained_delegation,
    username,
    callback
...
Kerberos.prototype.queryContextAttribute = function(attribute) {
  if (typeof attribute !== 'number' && attribute !== 0x00) {
...
if (kerberos.SecurityCredentials) {
  module.exports.SSPI = require('./sspi');
}
