...
static gss_client_response* gss_error(const char* func,
                                      const char* op,
                                      OM_uint32 err_maj,
                                      OM_uint32 err_min);
...
static gss_client_response* create_krb5_ccache(gss_server_state* state,
...
code)); goto end;
    ...
    while ((code = krb5_kt_next_entry(kcontext, kt, &entry, &cursor)) == 0)
    {
        ...
        {
            PyErr_SetObject(KrbException_class, Py_BuildValue("((s:i))",
                                                              "Cannot parse principal name from
keytab", code)); goto end;
        }
...
gss_client_response* authenticate_gss_client_unwrap(gss_client_state* state,
    ...
    // Do GSSAPI step
    maj_stat = gss_unwrap(&min_stat, state->context, &input_token, &output_token, NULL, NULL);
...
gss_client_response* authenticate_gss_client_wrap(gss_client_state* state,
    ...
    // Do GSSAPI wrap
    maj_stat = gss_wrap(
        &min_stat, state->context, 0, GSS_C_QOP_DEFAULT, &input_token, NULL, &output_token);
...
gss_client_response* authenticate_gss_server_init(const char* service,
    ...
    if (username != NULL) {
        ...
        maj_stat =
            gss_inquire_cred(&min_stat, state->client_creds, &state->client_name, NULL, NULL, NULL);
        if (GSS_ERROR(maj_stat)) {
...
gss_client_response* authenticate_user_krb5_password(const char* username,
    ...
    problem = krb5_get_init_creds_password(
        context, &creds, user_principal, (char*)password, NULL, NULL, 0, NULL, opt);
...
static gss_client_response* verify_krb5_kdc(krb5_context context,
    ...
    problem = krb5_rd_req(context, &auth_context, &req, server_princ, keytab, 0, NULL);
    if (problem) {
        response = krb5_ctx_error(context, problem);
        goto out;
    }
...
gss_client_response* gss_error(const char* func,
                               const char* op,
                               OM_uint32 err_maj,
                               OM_uint32 err_min) {
    OM_uint32 maj_stat, min_stat;
    ...
    do {
        ...
        n = snprintf(
            message, nleft, ": %.*s", (int)status_string.length, (char*)status_string.value);
        message += n;
        nleft -= n;
