...
void KerberosClient::UnwrapData(const CallbackInfo& info) {
    ...
    KerberosWorker::Run(
        callback, "kerberos:ClientUnwrap", [=](KerberosWorker::SetOnFinishedHandler onFinished) {
            sspi_result result = auth_sspi_client_unwrap(state.get(), (SEC_CHAR*)challenge.c_str());
...
void KerberosClient::WrapData(const CallbackInfo& info) {
    ...
    KerberosWorker::Run(
        callback, "kerberos:ClientWrap", [=](KerberosWorker::SetOnFinishedHandler onFinished) {
            sspi_result result = auth_sspi_client_wrap(state.get(),
                                                       (SEC_CHAR*)challenge.c_str(),
                                                       (SEC_CHAR*)user.c_str(),
                                                       (ULONG)user.length(),
                                                       protect);
...
void InitializeClient(const CallbackInfo& info) {
    ...
    ULONG gss_flags = flags_v.IsNumber() ? flags_v.As<Number>().Uint32Value()
    ...
    KerberosWorker::Run(
        ...
        [=](KerberosWorker::SetOnFinishedHandler onFinished) {
            ...
            sspi_result result = auth_sspi_client_init((WCHAR*)service.c_str(),
                                                       gss_flags,
                                                       (WCHAR*)user.c_str(),
                                                       (ULONG)user.length(),
                                                       (WCHAR*)domain.c_str(),
                                                       (ULONG)domain.length(),
                                                       (WCHAR*)password.c_str(),
                                                       (ULONG)password.length(),
                                                       (WCHAR*)mech_oid.c_str(),
                                                       client_state.get());
